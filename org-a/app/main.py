...
ORG_USERNAME = os.environ.get("ORG_USERNAME", "org_a")
ORG_PASSWORD = os.environ.get("ORG_PASSWORD", "changeme_a")
...
def sync_reports():
    ...
    for inc in incidents:
        if "evidence" not in inc:
            continue
        result = submit_for_custody(inc, sanitize_report(inc))
        if result:
            submitted += 1
...
def featurize(incident):
    """Turn one incident into a numeric feature vector. Only trained model
    weights ever leave this org -- never raw data like this."""
